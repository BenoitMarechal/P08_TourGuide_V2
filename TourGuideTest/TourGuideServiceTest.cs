...
namespace TourGuideTest
{
    public class TourGuideServiceTour : IClassFixture<DependencyFixture>
    {
        ...
        public async Task AddUser()
        {
            ...
            await _fixture.TourGuideService.AddUser(user);
            await _fixture.TourGuideService.AddUser(user2);
        ...
        public async Task GetTripDeals()
        {
            _fixture.Initialize(0);
            var user = new User(Guid.NewGuid(), "jon", "000", "jon@tourGuide.com");
            IEnumerable<Provider> providers = await _fixture.TourGuideService.GetTripDeals(user);
